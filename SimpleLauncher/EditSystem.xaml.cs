...
namespace SimpleLauncher
{
    public partial class EditSystem
    {
        ...
        private void SaveSystemButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            else if (formatsToSearch.Count == 0)
            {
                MessageBox.Show("The 'Format to Search in the Search Folder' is empty or is not valid.\n\nPlease fill this field.", "Alert", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }
            else if (extractFileBeforeLaunch == "true" && !formatsToSearch.All(f => f == "zip" || f == "7z" || f == "rar"))
            {
                MessageBox.Show("When 'Extract File Before Launch' is set to true, 'Format to Search in the System Folder' must include 'zip', '7z', or 'rar'.", "Validation Error", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }
