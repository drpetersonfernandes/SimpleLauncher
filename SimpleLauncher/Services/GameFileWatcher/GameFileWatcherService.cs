...
public sealed class GameFileWatcherService : IDisposable
{
    ...
    public void StartWatching(IEnumerable<string> folders, string systemName, IEnumerable<string> fileExtensions = null)
    {
        ...
        {
            DebugLogger.Log($"[GameFileWatcherService] No valid folders to watch for system '{systemName}'.");
            return;
        }
        ...
        lock (_lock)
        {
            foreach (var folder in resolvedFolders)
            {
                ...
                {
                    DebugLogger.Log($"[GameFileWatcherService] Failed to watch '{folder}': {ex.Message}");
                }
            }
        }
        ...
        lock (_lock)
        {
            DebugLogger.Log($"[GameFileWatcherService] Started watching {_watchers.Count} folder(s) for system '{systemName}'.");
        }
    }
    ...
    public void StopWatching()
    {
        lock (_lock)
        {
            foreach (var (watcher, _) in _watchers)
            {
                ...
                {
                    DebugLogger.Log($"[GameFileWatcherService] Error disposing watcher: {ex.Message}");
                }
            }
        ...
        DebugLogger.Log("[GameFileWatcherService] Stopped all watchers.");
    }
    ...
    private void OnFileChanged(object sender, FileSystemEventArgs e)
    {
        ...
        DebugLogger.Log($"[GameFileWatcherService] File change detected: {e.ChangeType} - {e.FullPath} (System: {tag.SystemName})");
    ...
    {
        DebugLogger.Log($"[GameFileWatcherService] Watcher error: {e.GetException().Message}");
    }
