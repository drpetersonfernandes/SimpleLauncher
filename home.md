...
| **ReceiveANotificationOnEmulatorError** | `true` or `false`. If `true`, Simple Launcher will show a message if the emulator exits with an error code.                                                             | `true`                                |
...
- **Search Bar:** Search for games within the currently selected system. Supports logical operators `AND` and `OR`.
...
- **Zoom In/Out:** Changes the size of game thumbnails in Grid View (also `Ctrl + Mouse Wheel`).
...
- **Custom Sound File:** Choose a custom `.mp3` file for notifications via **Options -> Sound Configuration -> Notification Sound File**.
- **Play Current Sound:** Test the currently configured sound.
